...
export default function App() {
  ...
  const addItem = (e?: React.FormEvent) => {
    ...
    setNewItemName('');
    // Mobilde klavyenin kapanmaması için odağı koruyoruz
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] flex justify-center transition-colors duration-300 select-none">
      <div className="w-full max-w-md relative flex flex-col h-[100dvh] overflow-hidden border-x border-black/5 dark:border-white/5 shadow-2xl">
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'list' && (
              <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {/* Form Yapısı Enter Tuşunu Aktif Eder */}
                <form onSubmit={addItem} className="mt-2 space-y-3">
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <button 
                        type="button" 
                        onClick={() => addItem()}
                        className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--primary-color)] active:scale-125 transition-transform z-10"
                      >
                      ...
                      <input 
                        ...
                        enterKeyHint="done"
                        className="w-full h-14 pl-12 pr-4 bg-black/5 dark:bg-white/5 rounded-2xl border-none outline-none focus:ring-2 focus:ring-primary/30 text-base"
                      />
                    </div>
                    <div className="relative">
                      <select 
                        ...
                        className="h-14 px-4 bg-black/5 dark:bg-white/5 rounded-2xl border-none outline-none text-xs font-black text-[var(--primary-color)] appearance-none min-w-[90px] dark:text-[#13ec5b]"
                      >
                  ...
                  <section key={cat} className="mt-6">
                    ...
                    <div className="space-y-3">
                      {activeItems.filter(i => i.category === cat).map(item => (
                        <div key={item.id} className="flex items-center justify-between card-bg p-4 rounded-2xl shadow-sm active:bg-black/5 dark:active:bg-white/5">
                          <div className="flex items-center gap-3" onClick={() => setItems(items.map(i => i.id === item.id ? {...i, completed: true} : i))}>
                            <div className="w-7 h-7 rounded-full border-2 border-primary/30 flex items-center justify-center transition-colors" />
                            <div>
                              <p className="font-bold text-sm">{item.name}</p>
                              <p className="text-[9px] text-[var(--primary-color)] font-black flex items-center gap-1 uppercase tracking-wider"><Store size={9} /> {item.market}</p>
                            </div>
                          </div>
                          <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="p-2 opacity-30 text-red-500"><X size={20}/></button>
                        </div>
                      ))}
                    </div>
                  </section>
                      ...
                      <div className="space-y-2 opacity-60">
                        {completedItems.map(item => (
                          <div key={item.id} className="flex items-center justify-between bg-black/5 dark:bg-white/5 p-3 rounded-xl">
                            <div className="flex items-center gap-3" onClick={() => setItems(items.map(i => i.id === item.id ? {...i, completed: false} : i))}>
                              ...
                              <span className="line-through text-xs font-bold">{item.name}</span>
                            </div>
                            <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="p-2 opacity-40"><Trash2 size={16}/></button>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            {view === 'history' && (
              <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pt-2">
                {history.map(trip => (
                  <div key={trip.id} className="card-bg rounded-2xl overflow-hidden transition-all active:scale-[0.98]">
                    <div onClick={() => setExpandedHistoryId(expandedHistoryId === trip.id ? null : trip.id)} className="p-5 relative">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2 text-[var(--primary-color)] font-black text-[10px] uppercase tracking-widest"><Store size={14} />{trip.store}</div>
                          ...
                          <div className="space-y-3">
                            {trip.detailedItems.map((detail, idx) => (
                              <div key={idx} className="flex justify-between items-center text-xs font-bold">
                                <div className="flex items-center gap-2"><Tag size={12} className="text-[var(--primary-color)]" /><span>{detail.name}</span></div>
                                <span className="text-[9px] bg-primary/20 text-[var(--primary-color)] px-2 py-1 rounded-md uppercase">{detail.market}</span>
                              </div>
                            ))}
                          </div>
                        </motion.div>
            ...
            {view === 'receipts' && (
              <motion.div key="receipts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pt-2">
                <button onClick={() => fileInputRef.current?.click()} className="w-full h-32 border-2 border-dashed border-primary/20 rounded-3xl flex flex-col items-center justify-center gap-2 text-[var(--primary-color)] bg-primary/5 active:bg-primary/10">
                  <Camera size={32} /> <span className="text-xs font-black">FİŞ FOTOĞRAFI EKLE</span>
                </button>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {receipts.map(r => (
                    <div key={r.id} className="card-bg rounded-2xl overflow-hidden relative aspect-square">
                      <img src={r.imageUrl} className="w-full h-full object-cover" alt="Fiş" />
                      <button onClick={() => setReceipts(receipts.filter(x => x.id !== r.id))} className="absolute top-2 right-2 bg-black/60 p-2 rounded-full text-white"><Trash2 size={16}/></button>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pt-2">
                <section>
                  ...
                  <div className="card-bg p-4 rounded-2xl flex items-center justify-between" onClick={() => setIsDarkMode(!isDarkMode)}>
                    ...
                    <div className={`w-14 h-8 rounded-full relative transition-colors duration-300 ${isDarkMode ? 'bg-[#13ec5b]' : 'bg-slate-300'}`}>
                      <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all duration-300 ${isDarkMode ? 'left-7' : 'left-1'}`} />
                    </div>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="flex gap-2 mb-4">
                    <input type="text" value={newMarketName} onChange={(e) => setNewMarketName(e.target.value)} placeholder="Yeni market..." className="flex-1 h-12 px-4 bg-black/5 dark:bg-white/5 rounded-xl outline-none text-sm" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {markets.map(m => (
                      <div key={m} className="bg-black/5 dark:bg-white/5 p-3 rounded-xl flex items-center justify-between">
                        ...
                        {m !== 'Genel' && <button onClick={() => setMarkets(markets.filter(item => item !== m))} className="p-1 opacity-30 text-red-500"><X size={16}/></button>}
                      </div>
                    ))}
                  </div>
                </section>
        ...
        {/* Mobil Navigasyon - Sabit ve Dokunmatik Dostu */}
